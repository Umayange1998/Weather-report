...
const sendEmail = async (userName, weatherData) => {
  const mailOptions = {
    ...
    to: userName, // Receiver's email 
    subject: "Hourly Weather Report",
    text: JSON.stringify(weatherData, null, 2), // Send raw weather data 
  };
...
const scheduleWeatherUpdates = (user) => {
  cron.schedule("0 */3 * * *", async () => {
    ...
    await updateUserWeatherData(user, user.location);
  });
};
